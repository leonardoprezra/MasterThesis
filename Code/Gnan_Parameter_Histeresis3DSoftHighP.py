...
nameFormat = "dataFLEXHighP_{poly}/" + settings['nameString']
...
for initDict in parameterspace:
    ...
    # Create directories
    try:
        if(not os.path.exists("dataFLEXHighP_{poly}/".format(**initDict))):
            os.mkdir("dataFLEXHighP_{poly}/".format(**initDict))
    except OSError as e:
        if e.errno != 17:
            raise
        pass
    ...
    if(os.path.exists(nameString+"_Stage-1.outputs")):
        ...
        if(not os.path.exists(nameString+"_Stage-2.outputs")) and (not os.path.exists(nameString+"_Stage-3.outputs")):
            ...
            print("\nStage-4")
            print(out_file)
        elif(os.path.exists(nameString+"_Stage-4.outputs")) and (not os.path.exists(nameString+"_Stage-5.outputs")):
            out_file = nameString+"_Stage-5.outputs"
            print("\nStage-5")
            print(out_file)
        else:
            print("\nSkipping "+nameString)
    ...
    proc = subprocess.Popen(["python",  "-u",
                             # "MarsonNVE.py",  #
                             "/home/hpc/iwsp/iwsp023h/MasterThesis/Code/GnanNVEHighP.py",
                             # "/nishome/students/leonardo/Dokumente/Thesis/Code/GnanNVEHighP.py",
                             # "/home/leo/MasterThesis/Code/GnanNVEHighP.py",
                             *initString],
                            stdout=out,
                            stderr=out)
    proc.wait()
