...
nameString = "dataFLEXLongerT_{}/".format(
    poly_key) + settings['nameString'].format(**settings)

settings['N'] = user_N

# Create directory to store simulation results
if(not os.path.exists("dataFLEXLongerT_{}".format(poly_key))):
    os.mkdir("dataFLEXLongerT_{}".format(poly_key))
...
# #
# #
# Checks presence/absence of Stage gsd files
stage_1 = True
stage_2 = False
stage_3 = False
stage_4 = False
...
restart_name = nameString+"_Stage-1.gsd"

if(os.path.exists(nameString+"_Longer-1.gsd")):
    stage_1 = False
    stage_2 = True
    stage_3 = False
    stage_4 = False
    stage_5 = False
    restart_name = nameString+"_Stage-2.gsd"

if(os.path.exists(nameString+"_Longer-2.gsd")):
    stage_1 = False
    stage_2 = False
    stage_3 = True
    stage_4 = False
    stage_5 = False
    restart_name = nameString+"_Stage-3.gsd"

if(os.path.exists(nameString+"_Longer-3.gsd")):
    stage_1 = False
    stage_2 = False
    stage_3 = False
    stage_4 = True
    stage_5 = False
    restart_name = nameString+"_Stage-4.gsd"

if(os.path.exists(nameString+"_Longer-4.gsd")):
    stage_1 = False
    stage_2 = False
    stage_3 = False
    stage_4 = False
    stage_5 = True
    restart_name = nameString+"_Stage-5.gsd"
...
if restart_avail and stage_1:
    ...
    gsd = hoomd.dump.gsd(filename='{:s}_Longer-1.gsd'.format(nameString),
                    period=outputInterval_gsd,
                    group=hoomd.group.all(),
                    dynamic=['momentum'],
                    phase=0)

    log = hoomd.analyze.log(filename='{:s}_Longer-1.log'.format(nameString),
                            quantities=['volume',
                                        'momentum',
                                        'time',
                                        'potential_energy',
...
if restart_avail and stage_2:
    ...
    gsd = hoomd.dump.gsd(filename='{:s}_Longer-2.gsd'.format(nameString),
                    period=outputInterval_gsd,
                    group=hoomd.group.all(),
                    dynamic=['momentum'],
                    phase=0)

    log = hoomd.analyze.log(filename='{:s}_Longer-2.log'.format(nameString),
                            quantities=['volume',
                                        'momentum',
                                        'time',
                                        'potential_energy',
...
if restart_avail and stage_3:
    ...
    gsd = hoomd.dump.gsd(filename='{:s}_Longer-3.gsd'.format(nameString),
                    period=outputInterval_gsd,
                    group=hoomd.group.all(),
                    dynamic=['momentum'],
                    phase=0)

    log = hoomd.analyze.log(filename='{:s}_Longer-3.log'.format(nameString),
                            quantities=['volume',
                                        'momentum',
                                        'time',
                                        'potential_energy',
...
if restart_avail and stage_4:
    ...
    gsd = hoomd.dump.gsd(filename='{:s}_Longer-4.gsd'.format(nameString),
                    period=outputInterval_gsd,
                    group=hoomd.group.all(),
                    dynamic=['momentum'],
                    phase=0)

    log = hoomd.analyze.log(filename='{:s}_Longer-4.log'.format(nameString),
                            quantities=['volume',
                                        'momentum',
                                        'time',
                                        'potential_energy',
...
if restart_avail and stage_5:
    print('!!!!!!!!!!!!!!!!!!!!!\n5th Long Simulation')
    dens = vol/system.box.get_volume()
    print('Density={}'.format(dens))

    gsd = hoomd.dump.gsd(filename='{:s}_Longer-5.gsd'.format(nameString),
                    period=outputInterval_gsd,
                    group=hoomd.group.all(),
                    dynamic=['momentum'],
                    phase=0)

    log = hoomd.analyze.log(filename='{:s}_Longer-5.log'.format(nameString),
                            quantities=['volume',
                                        'momentum',
                                        'time',
                                        'potential_energy',
