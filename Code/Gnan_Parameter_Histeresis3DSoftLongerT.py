...
settings['outputInterval_gsd'] = 1500
...
nameFormat = "dataFLEXLongerT_{poly}/" + settings['nameString']
...
for initDict in parameterspace:
    ...
    # Create directories
    try:
        if(not os.path.exists("dataFLEXLongerT_{poly}/".format(**initDict))):
            os.mkdir("dataFLEXLongerT_{poly}/".format(**initDict))
    except OSError as e:
        if e.errno != 17:
            raise
        pass
    ...
    if(os.path.exists(nameString+"_Longer-1.outputs")):

        if(not os.path.exists(nameString+"_Longer-2.outputs")) and (not os.path.exists(nameString+"_Longer-3.outputs")):
            out_file = nameString+"_Longer-2.outputs"
            print("\nLonger-2")
            print(out_file)
        elif(os.path.exists(nameString+"_Longer-2.outputs")) and (not os.path.exists(nameString+"_Longer-3.outputs")):
            out_file = nameString+"_Longer-3.outputs"
            print("\nLonger-3")
            print(out_file)
        elif(os.path.exists(nameString+"_Longer-3.outputs")) and (not os.path.exists(nameString+"_Longer-4.outputs")):
            out_file = nameString+"_Longer-4.outputs"
            print("\nLonger-4")
            print(out_file)
        elif(os.path.exists(nameString+"_Longer-4.outputs")) and (not os.path.exists(nameString+"_Longer-5.outputs")):
            out_file = nameString+"_Longer-5.outputs"
            print("\nLonger-5")
            print(out_file)
        else:
            print("\nSkipping "+nameString)
        ...
        out_file = nameString+"_Longer-1.outputs"
        print("\nLonger-1")
        print(out_file)
    ...
    proc = subprocess.Popen(["python",  "-u",
                             # "MarsonNVE.py",  #
                             "/home/hpc/iwsp/iwsp023h/MasterThesis/Code/GnanNVELongerT.py",
                             # "/nishome/students/leonardo/Dokumente/Thesis/Code/GnanNVELongerT.py",
                             # "/home/leo/MasterThesis/Code/GnanNVELongerT.py",
                             *initString],
                            stdout=out,
                            stderr=out)
    proc.wait()
